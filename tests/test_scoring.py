...
def test_no_anchor_is_wrong_on_seed_42(baseline):
    ...
    the candidates. The gap between 97 recovered and 64 closed is not parsing.
    ...
    **97 where stage 11 measured 101.** Four of the six lines stage 15's
    reversal-pair exclusion withholds used to recover an anchor, which is the census
    reading better rather than worse: an anchor on a duplicate posting is a
    settlement id parsed off a line that is not a payout, and it was counted as a
    recovery. The load-bearing half did not move.
    """
    data, truth, _, trace, _ = baseline
    got = anchors_recovered(trace, truth,
                            {t.entity_id: t.settlement_id for t in data.txns})
    assert got["recovered"] == 97 and got["wrong"] == 0
...
def test_the_scoreboard_renders(baseline):
    ...
    assert "anchors recovered 97" in text
